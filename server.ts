...
function dbQuery(fn: () => any, fallback: any = []) {
...
app.post('/api/attendance/clock-in', (req, res) => {
  ...
  const result = dbQuery(() => db.prepare('INSERT INTO attendance (user_id, date, clock_in) VALUES (?, ?, ?)').run(user_id, today, new Date().toISOString()), null);
  res.json(result ? { id: result.lastInsertRowid } : { error: 'DB unavailable' });
});

app.post('/api/attendance/clock-out', (req, res) => {
  const { id } = req.body;
  dbQuery(() => db.prepare('UPDATE attendance SET clock_out = ? WHERE id = ?').run(new Date().toISOString(), id));
...
app.post('/api/email/send', (req, res) => {
  ...
  dbQuery(() => db.prepare('INSERT INTO interactions (client_id, type, content) VALUES (?, ?, ?)').run(
    client_id, 'email', `KIKÜLDÖTT EMAIL: ${subject}\n\n${body}`
  ));
  res.json({ success: true, message: 'Email elküldve (szimulált)' });
});

// Invoicing Placeholders
